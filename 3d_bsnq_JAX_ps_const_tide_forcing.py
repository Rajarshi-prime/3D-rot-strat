...
m = [10,15,20][int(float(sys.argv[-1]))] # Dissipation strength at the highest kmax. 
...
if nu!= 0: savePath = curr_path/f"data/bsnq/f_{f_corr:.1f}_Nb_{N_b:.1f}/tide_forced_{isforcing}/N_{N}_Re_{re:.1f}"
else: savePath = curr_path/f"data/bsnq/tide_forced_{isforcing}/N_{N}_Re_inf"
...
phase_k = np.exp(1j*(kvec[0]*dx/2 + kvec[1]*dy/2 + kvec[2]*dz/2))*dealias
...
@jax.jit
def forcing(tt,uk,bk,f0 = f0*nshells,cond = cond ,h = dt,invlap = invlap,N_b = N_b, f_corr = f_corr):
    ...
    f1uk = (pk*np.exp(1j*sig*tt))[None,...]*np.array([(1j*ky*f_corr - kx*sig)*denom1,-(1j*kx + ky*sig)*denom1, kz*sig*denom2])
    ...
    neg_corr = np.sum(normalize*(np.einsum('i...,i...->...',np.conjugate(uk),f1uk) + np.conjugate(bk)*f1bk).real)
    # ---------------------------------------------------------- #
    # ------------------- positive frequency ------------------- #
    theta = np.random.uniform(0,TWO_PI,(N,Np,Nf))
    pk = ensure_reality(np.exp(1j*theta)*cond*N**3*dealias)
    ...
    f2uk = (pk*np.exp(1j*sig*tt))[None,...]*np.array([(1j*ky*f_corr - kx*sig)*denom1,-(1j*kx + ky*sig)*denom1, kz*sig*denom2])
    ...
    pos_corr = np.sum(normalize*(np.einsum('i...,i...->...',np.conjugate(uk),f2uk) + np.conjugate(bk)*f2bk).real)
    
    # ---------------------------------------------------------- #
    norm = np.sum(normalize*(np.einsum('i...,i...->...',np.conjugate(f1uk),f1uk) + np.conjugate(f1bk)*f1bk).real)**0.5
    ...
    alpha = (2.0*f0/np.sum(normalize*(np.einsum('i...,i...->...',np.conjugate(fk),fk) + np.conjugate(fkb)*fkb).real)/h)**0.5
...
def RHS(uk, bk,visc = 1,forc = 1,isexplicit = isexplicit,invlap = invlap,fvec = fvec, N_bvec = N_bvec):
    ...
    rhsk = 0.5*rfft(np.einsum('ijk,j...,k...->i...', epsilon, u,omg ))*dealias[None,...]

    rhsbk = 0.5*(-1j*np.einsum('i...,i...->...',kvec,rfft(u*irfft(bk)[None,...]))
    )*dealias


    u = irfft(uk*phase_k[None,...])
    omg = irfft(1j*np.einsum('ijk,j...,k...->i...',epsilon,kvec,uk)*phase_k[None,...])
    ...
    rhsk +=   (N_bvec[:,None,None,None]*bk[None, ...] )*dealias[None,...]
    rhsbk += (- N_b*uk[2] )*dealias
...
def RK4(i,h,ti,uk,bk,semi_G_half, semi_G, hypervisc):
    ...
    fk,fkb = forcing(ti,uknew*hypervisc,bk*hypervisc)
    """ Enforcing the reality condition """
    u = irfft(uknew*hypervisc + h*fk)
    b = irfft(bknew*hypervisc + h*fkb)
