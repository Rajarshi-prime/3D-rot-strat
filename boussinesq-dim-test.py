...
## ------------- Time steps --------------
N = 128
dt = 0.256/(N*3/2)*(2*np.pi)**(3/2)  #! Such that increasing resolution will decrease the dt
T = 20*(2*np.pi)**(3/2)*5
dt_save = 1.0
st = round(dt_save/dt)
## ---------------------------------------
...
# nu = nu0/(N**(4/3))  
# ro = 1 # Rossby number
f_corr = 0 # Coriolis parameter
N_b = 0 #Buoyancy freqency
fbyN = f_corr/N_b if N_b !=0 else 0.0 
einit = 10 # Initial energy
...
f0 = 0.2/ nshells #! Total power input at each shells
# f0 = nu0**3/ nshells #! Total power input at each shells
...
if nu!= 0: savePath = pathlib.Path(f"./data/bsnq_phs_shifted/f_{f_corr}_Nb_{N_b}/forced_{isforcing}/N_{N}_Re_{re:.1f}")
else: savePath = pathlib.Path(f"./data/bsnq_phs_shifted/forced_{isforcing}/N_{N}_Re_inf")
...
# dealias = kint<=2**0.5*N//3 #! Phase-shift spherical dealiasing
dealias = (kint<=N//2)*(np.abs(kx + ky)<2*N/3)*(np.abs(kx - ky)<2*N/3)*(np.abs(kx + kz)<2*N/3)*(np.abs(kx - kz)<2*N/3)*(np.abs(ky + kz)<2*N/3)*(np.abs(ky - kz)<2*N/3) #! Phase-shift dealiasing
...
def convolve(uk,bk):
    ...
    rhsuk[:]  = (rfft_mpi(rhsu, rhsuk) )*dealias/2 
    rhsvk[:]  = (rfft_mpi(rhsv, rhsvk) )*dealias/2 
    rhswk[:]  = (rfft_mpi(rhsw, rhswk) )*dealias/2
    rhsbk[:] = (rfft_mpi(rhsb,rhsbk) )*dealias/2
     
    
    
    u[0] = irfft_mpi(uk[0]*dealias*phase_k, u[0])
    u[1] = irfft_mpi(uk[1]*dealias*phase_k, u[1])
    u[2] = irfft_mpi(uk[2]*dealias*phase_k, u[2])
    b[:] = irfft_mpi(bk*dealias*phase_k, b)
    
    
    omg[0] = irfft_mpi(1j*(ky*uk[2] - kz*uk[1])*dealias*phase_k,omg[0])
    omg[1] = irfft_mpi(1j*(kz*uk[0] - kx*uk[2])*dealias*phase_k,omg[1])
    omg[2] = irfft_mpi(1j*(kx*uk[1] - ky*uk[0])*dealias*phase_k,omg[2])
    ...
    rhsuk += (rfft_mpi(rhsu, rhsuk) )*(1/phase_k)*dealias/2 
    rhsvk += (rfft_mpi(rhsv, rhsvk) )*(1/phase_k)*dealias/2 
    rhswk += (rfft_mpi(rhsw, rhswk) )*(1/phase_k)*dealias/2 
    rhsbk += (rfft_mpi(rhsb,rhsbk) )*(1/phase_k)*dealias/2
    
    
    return rhsuk, rhsvk, rhswk ,rhsbk
...
def forcing(uk,bk):
    ...
    ek[:] = 0.5*(np.abs(uk_w[0])**2 + np.abs(uk_w[1])**2 + np.abs(uk_w[2])**2 + np.abs(bk_w)**2)*dealias*normalize #! This is the 3D ek array of waves
...
def RHS(uk, bk,uk_t,bk_t,visc = 1,forc = 1):
    ...
    # rhsbk[:] = (rfft_mpi(rhsb,rhsbk) + fkb)*dealias
    
    rhsuk[:], rhsvk[:], rhswk[:] ,rhsbk[:] = convolve(uk,bk)
    ## The pressure term
    ...
    uk_t[0] = rhsuk - 1j*kx*pk + f_corr*uk[1] - nu*((-lap)**lp)*uk[0]*isexplicit * visc + fk[0]
    uk_t[1] = rhsvk - 1j*ky*pk - f_corr*uk[0] - nu*((-lap)**lp)*uk[1]*isexplicit * visc + fk[1]
    uk_t[2] = rhswk - 1j*kz*pk + bk*N_b - nu*((-lap)**lp)*uk[2]*isexplicit * visc + fk[2]
    bk_t[:] = rhsbk - N_b*uk[2] - nu*((-lap)**lp)*bk*isexplicit * visc + fkb
...
#----------------- testing convolve ----------
# rhsuk[:], rhsvk[:], rhswk[:], rhsbk[:] = convolve(uk,bk)
# rhsmax = comm.allreduce( np.max( normalize**0.5*(np.abs(rhsuk) + np.abs(rhsvk) + np.abs(rhswk) + np.abs(rhsbk)) ), op = MPI.MAX)
# if rank ==0 : print(f"Max value of the convolved RHS terms {rhsmax}")
# raise SystemExit
#-------------------------------------------------------
