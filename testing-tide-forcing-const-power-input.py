...
if nu!= 0: savePath = pathlib.Path(f"./data/bsnq/f_{f_corr:.1f}_Nb_{N_b:.1f}/tide_forced_{isforcing}/N_{N}_Re_{re:.1f}")
else: savePath = pathlib.Path(f"./data/bsnq/tide_forced_{isforcing}/N_{N}_Re_inf")
...
cond = (np.abs(kx) == 1.0)*(np.abs(ky) == 1.0)*(np.abs(kz) == 1.0)
...
vis = 0.0*nu*(k)**(2*lp) ## This is in Fourier Space
...
def RHS(uk, bk,uk_t,bk_t,visc = 1,forc = 1,rhsuk = rhsuk, rhsvk = rhsvk, rhswk = rhswk, rhsbk = rhsbk):
    ...
    rhsuk[:]  = (0.0*rfft_mpi(rhsu, pk)*conjphase_k + f_corr*uk[1] )*dealias 
    rhsvk[:]  = (0.0*rfft_mpi(rhsv, pk)*conjphase_k - f_corr*uk[0] )*dealias 
    rhswk[:]  = (0.0*rfft_mpi(rhsw, pk)*conjphase_k + bk*N_b )*dealias 
    rhsbk[:] = (0.0*rfft_mpi(rhsb,pk)*conjphase_k - N_b*uk[2])*dealias
    ...
    uk_t[0] = rhsuk - 1j*kx*pk - nu*((-lap)**lp)*uk[0]*isexplicit * visc*0.0
    uk_t[1] = rhsvk - 1j*ky*pk - nu*((-lap)**lp)*uk[1]*isexplicit * visc*0.0
    uk_t[2] = rhswk - 1j*kz*pk - nu*((-lap)**lp)*uk[2]*isexplicit * visc*0.0
    bk_t[:] = rhsbk - nu*((-lap)**lp)*bk*isexplicit * visc*0.0
...
## ------------- Evolving the system ----------------- 
def evolve_and_save(t,  u,uknew = uknew, bknew = bknew): 
    global begin
    h = t[1] - t[0]
    ...
    for i in range(t.size-1):
        ...
        ## ------------- saving the data -------------------- ##
        if i % st ==0 :
            if rank ==0: print(f"Expected energy {f0*nshells*t[i]}")
        #     save_hdf5(i,uk,bk)
            save(i,uk,bk)
        begin = True   
        ## -------------------------------------------------- ##
        ...
        eb4 = 0.5*comm.allreduce(np.sum(normalize*((np.abs(uk)**2).sum(axis = 0) + np.abs(bk)**2)),op =MPI.SUM)
        # fk[:] = forcing(uknew,fk)
        # fk[:],fkb[:] = forcing(t[i],uk,bk)
        k1u[:],k1b[:] = RHS(uk,bk, k1u,k1b)
        # corrk1 = comm.allreduce(np.sum(normalize*(np.conjugate(uk[0])*k1u[0] + np.conjugate(uk[1])*k1u[1] + np.conjugate(uk[2])*k1u[2] +np.conjugate(bk)*k1b).real),op = MPI.SUM)
        # k2u[:],k2b[:] = RHS(uk + h*k1u,bk + h*k1b ,k2u,k2b) #! Only for RK2
        k2u[:],k2b[:] = RHS(uk + h/2.*k1u ,bk + h/2.*k1b,k2u,k2b)
        k3u[:],k3b[:] = RHS(uk + h/2.*k2u, bk + h/2.*k2b,k3u,k3b)
        k4u[:],k4b[:] = RHS(uk + h*k3u,bk + h*k3b, k4u,k4b)
        
        # uknew[:] = uk + h/2.0* ( k1u + k2u )  
        uknew[:] = (uk + h/6.0* ( k1u + 2*(k2u + k3u) + k4u)  )*hypervisc 
        bknew[:] = (bk + h/6.0* ( k1b + 2*(k2b + k3b) + k4b)  )*hypervisc 



        if t[i]>-1: fk[:],fkb[:] = forcing(t[i],uknew,bknew)
        else : fk[:],fkb[:] = 0.0, 0.0
        
        coornew = comm.allreduce(np.sum(normalize*(np.einsum('i...,i...->...',np.conjugate(uknew),fk) + np.conjugate(bknew)*fkb).real), op = MPI.SUM)
        
        uknew += fk*h*dealias 
        bknew += fkb*h*dealias
        
        # uk_v[:],bk_v[:] = vortex(fk,fkb)
        # maxvals = comm.allreduce(max(np.max(np.abs(uk_v)**2*normalize),np.max(np.abs(bk_v)**2*normalize)),op = MPI.MAX)
        # if rank ==0: print(f"Max of vortex part {maxvals**0.5}")
        # RHS(fk,fkb,k1u,k1b)
        # raise SystemExit
        
        enow = 0.5*comm.allreduce(np.sum(normalize*((np.abs(uknew)**2).sum(axis = 0) + np.abs(bknew)**2)),op =MPI.SUM)
        ediff = (enow -eb4)

        
        # if rank ==0 : print(f'Injected energy {ediff/h/TWO_PI**3}, expected {f0/TWO_PI**3}, corr {coornew}')
